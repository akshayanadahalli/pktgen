...
function usage() {
    ...
    echo "  -x : (\$DEBUG)     debug"
    echo "  -6 : (\$IP6)       IPv6"
    echo "  -p : (\$PPS)       PPS per thread"
    echo "  -S : (\$START)     Start Port"
    echo "  -E : (\$END)       End Port"
    echo ""
}
...
while getopts "s:i:d:m:f:t:c:n:p:S:E:b:vxh6" option; do
    case $option in
        i) # interface
          export DEV=$OPTARG
	...
	p)
	  ...
	  ;;
	S)
	  export UDP_SP_MIN=$OPTARG
          export UDP_DP_MIN=$UDP_SP_MIN
	  info "UDP_SP_MIN=$UDP_SP_MIN"
	  info "UDP_DP_MIN=$UDP_SP_MIN"
	  ;;
	E)
	  export UDP_SP_MAX=$OPTARG
          export UDP_DP_MAX=$UDP_SP_MAX
	  info "UDP_SP_MAX=$UDP_SP_MAX"
	  info "UDP_DP_MAX=$UDP_SP_MAX"
	  ;;
        h|?|*)
          usage;
